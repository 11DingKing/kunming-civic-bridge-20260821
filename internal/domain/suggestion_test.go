...
func TestItem_IsOverdue(t *testing.T) {
	...
	item := &Suggestion{Deadline: now.Add(-1 * time.Hour), Status: StatusInProgress}
	assert.True(t, item.IsOverdue(now))
	assert.False(t, item.IsOverdue(now.Add(-2*time.Hour)))

	completed := &Suggestion{Deadline: now.Add(-1 * time.Hour), Status: StatusCompleted}
	assert.False(t, completed.IsOverdue(now))
}

func TestItem_Validate(t *testing.T) {
	item := &Suggestion{ExternalRef: "REF-001", Title: "Test", RegisteredBy: "user1", Deadline: time.Now()}
	assert.NoError(t, item.Validate())

	bad := &Suggestion{ExternalRef: "", Title: "Test", RegisteredBy: "user1", Deadline: time.Now()}
	err := bad.Validate()
	require.Error(t, err)
...
func TestRule_Matches(t *testing.T) {
	item := &Suggestion{Category: "tax", Keywords: []string{"property", "refund"}}
	rule := &Rule{
		MatchCategory: "tax",
