...
func makeItem(id, ref string, now time.Time) *domain.Suggestion {
...
func saveItemWithAssignment(t *testing.T, ctx context.Context, st *Store, item *domain.Suggestion) {
	t.Helper()
...
func TestRepo_PersistAndRecoverRestart(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "Suggestion restart-1", loaded.Title)
...
func TestRepo_TransactionCommitRollback(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "Suggestion commit-1", loaded.Title)
...
func TestRepo_ConcurrentRaceParallel(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		assert.Equal(t, fmt.Sprintf("Suggestion race-%d", i), item.Title)
	}
...
func TestRepo_RebuildIndexReplay(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		assert.Equal(t, fmt.Sprintf("Suggestion rebuild-%d", i), item.Title)
	}
}
