...
func (t *Tx) InsertItem(ctx context.Context, item *domain.Suggestion) error {
	_, err := t.tx.ExecContext(ctx, `INSERT INTO items (
		id, external_ref, title, description, citizen_name, citizen_contact,
		...
		item.CitizenName, item.CitizenContact, encodeSlice(item.Materials),
		...
		item.RegisteredBy, formatTime(item.Deadline), item.EscalationLevel, item.CollectionPointID,
...
func (t *Tx) UpdateItem(ctx context.Context, item *domain.Suggestion) error {
	result, err := t.tx.ExecContext(ctx, `UPDATE items SET
		title=?, description=?, citizen_name=?, citizen_contact=?,
		materials=?, category=?, keywords=?, status=?,
		lead_department=?, co_departments=?, rule_version=?,
		deadline=?, escalation_level=?, collection_point_id=?,
		completed_at=?, cancelled_at=?, cancel_reason=?,
		data_version=?, updated_at=? WHERE id=?`,
		item.Title, item.Description, item.CitizenName, item.CitizenContact,
		...
		item.CollectionPointID, formatNullableTime(item.CompletedAt),
...
func (t *Tx) InsertBatch(ctx context.Context, b *domain.ImportBatch) error {
	_, err := t.tx.ExecContext(ctx, `INSERT INTO import_batches (
		id, collection_point_id, batch_date, total_rows, success_count, failure_count,
		imported_at, shard_path, data_version
	) VALUES (?,?,?,?,?,?,?,?,?)`,
		b.ID, b.CollectionPointID, formatTime(b.BatchDate), b.TotalRows,
