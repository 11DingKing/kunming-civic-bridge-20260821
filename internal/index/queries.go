...
const itemCols = `id, external_ref, title, description, citizen_name, citizen_contact,
	materials, category, keywords, status, lead_department, co_departments, rule_version,
	registered_at, submitted_by, deadline, escalation_level, collection_point_id,
	completed_at, cancelled_at, cancel_reason, shard_path, shard_offset, data_version,
	created_at, updated_at`

func scanItem(row scanner) (*domain.Suggestion, error) {
	var (
...
const batchCols = `id, collection_point_id, batch_date, total_rows, success_count, failure_count,
...
func scanBatch(row scanner) (*domain.ImportBatch, error) {
	...
	err := row.Scan(
		&b.ID, &b.CollectionPointID, &bd, &b.TotalRows, &b.SuccessCount,
...
func (i *Index) GetItemByID(ctx context.Context, id string) (*domain.Suggestion, error) {
...
func (i *Index) GetItemByExternalRef(ctx context.Context, ref string) (*domain.Suggestion, error) {
...
func (i *Index) ListItems(ctx context.Context, filter domain.ItemFilter) ([]*domain.Suggestion, int, error) {
	where := []string{"1=1"}
	args := []interface{}{}
	if filter.Status != "" {
	...
	if filter.CollectionPointID != "" {
		where = append(where, "collection_point_id = ?")
		args = append(args, filter.CollectionPointID)
	}
...
func (i *Index) FindOverdueItems(ctx context.Context, now time.Time, maxLevel int) ([]*domain.Suggestion, error) {
