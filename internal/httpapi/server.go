...
import (
	...
	"github.com/11DingKing/kunming-civic-bridge-20260821/internal/worker"
)
...
func New(cfg *config.Config, st store.Store, clk clock.Clock, logger *applog.Logger, sched *scheduler.Scheduler) *Server {
	...
	srv := &Server{
		...
		civicSvc:     service.NewCivicService(st, clk),
...
func (s *Server) setupRouter() {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/suggestions", func(r chi.Router) {
			...
			r.Post("/{id}/intake", s.intakeSuggestion)
			r.Post("/{id}/reviews", s.assignReview)
			r.Get("/{id}/reviews", s.listSuggestionReviews)
			r.Post("/{id}/handling-plans", s.createHandlingPlan)
			r.Get("/{id}/handling-plans", s.listHandlingPlans)
			r.Post("/{id}/conversion", s.proposeConversion)
			r.Post("/{id}/conversion/verify", s.verifyConversion)
			r.Post("/{id}/feedback", s.queueFeedback)
		})
		r.Route("/campaigns", func(r chi.Router) {
			r.With(s.requireRoles(auth.RoleAdmin)).Post("/", s.createCampaign)
			r.Get("/", s.listCampaigns)
			r.With(s.requireRoles(auth.RoleAdmin)).Post("/{id}/transition", s.transitionCampaign)
			r.With(s.requireRoles(auth.RoleAdmin)).Post("/{id}/collection-points", s.addCollectionPoint)
			r.Get("/{id}/collection-points", s.listCollectionPoints)
			r.With(s.requireRoles(auth.RoleAdmin, auth.RoleReviewer)).Post("/{id}/advisors", s.enrollAdvisor)
			r.Get("/{id}/advisors", s.listAdvisors)
		})
		r.With(s.requireRoles(auth.RoleAdmin)).Post("/collection-points/{id}/activate", s.activateCollectionPoint)
		r.With(s.requireRoles(auth.RoleReviewer)).Post("/reviews/{id}/decision", s.decideReview)
		r.With(s.requireRoles(auth.RoleAdmin, auth.RoleHandler)).Post("/handling-plans/{id}/transition", s.transitionHandlingPlan)
		r.Route("/rules", func(r chi.Router) {
			r.With(s.requireRoles(auth.RoleAdmin, auth.RoleReviewer)).Post("/", s.createRule)
			r.Get("/", s.listRules)
			r.Get("/{version}", s.getRule)
		})
		r.Route("/batches", func(r chi.Router) {
			r.With(s.requireRoles(auth.RoleAdmin, auth.RoleReviewer)).Post("/import", s.batchImport)
