...
func TestLoginExpiryAndLogoutRevocationPersist(t *testing.T) {
	...
	user, token, expires, err := st.Login("reviewer", "test-prosecutor-password", time.Hour, now)
	if err != nil {
		t.Fatal(err)
	}
	if user.Role != RoleReviewer || !expires.Equal(now.Add(time.Hour)) {
...
func TestExpiredSessionAndRoleGuard(t *testing.T) {
	...
	now := time.Date(2026, 8, 19, 12, 0, 0, 0, time.UTC)
	user, token, _, err := st.Login("handler", "test-counselor-password", time.Minute, now)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := RequireRole(user, RoleReviewer); !errors.Is(err, ErrForbidden) {
...
func testUsers() []BootstrapUser {
	return []BootstrapUser{
		...
		{ID: "u-prosecutor", Username: "reviewer", Password: "test-prosecutor-password", Role: RoleReviewer},
		{ID: "u-counselor", Username: "handler", Password: "test-counselor-password", Role: RoleHandler},
	}
}
