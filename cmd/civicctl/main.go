...
func printUsage() {
	fmt.Fprintln(os.Stderr, `civicctl - operations CLI for the Kunming public suggestion service

Usage: civicctl <command> [flags]

Commands:
  init           Initialize data directory and schema
  import         Import public suggestions from a JSON file
  export         Export public suggestions to a JSONL file
...
func runInit(ctx context.Context, cfg *config.Config, clk clock.Clock, logger *applog.Logger) {
	...
	rule := &service.CreateRuleRequest{
		...
		Description:    "default fallback rule for unclassified public suggestions",
...
func runExport(ctx context.Context, cfg *config.Config, clk clock.Clock, logger *applog.Logger, fs *flag.FlagSet) {
	...
	fmt.Fprintf(os.Stderr, "export complete: %d suggestions\n", total)
}
